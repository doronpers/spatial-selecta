# Documentation Index

Complete catalog of all documentation for SpatialSelects.com.

## 📖 Essential Reading

| Document | Description | Audience |
|----------|-------------|----------|
| [README](../README.md) | Project overview and quick start | Everyone |
| [CONTRIBUTING](../CONTRIBUTING.md) | Contribution guidelines | Contributors |
| [CHANGELOG](../CHANGELOG.md) | Version history | Everyone |

## 🚀 Guides

| Document | Description |
|----------|-------------|
| [Setup Guide](Guides/SETUP.md) | Local development setup |
| [Deployment Guide](Guides/DEPLOYMENT.md) | Production deployment to Render.com |
| [Docker Guide](Guides/DOCKER.md) | Docker deployment with compose |
| [Development Guide](Guides/DEVELOPMENT.md) | Development workflow and practices |

## 📚 Reference

| Document | Description |
|----------|-------------|
| [API Reference](Reference/API.md) | REST API endpoints and usage |
| [Data Format](Reference/DATA_FORMAT.md) | Track data structure and validation |
| [Security](Reference/SECURITY.md) | Security practices and guidelines |

## 🎯 Governance

| Document | Description |
|----------|-------------|
| [Agent Guidelines](governance/AGENTS.md) | Comprehensive guide for AI agents (consolidated) |
| [Documentation Standards](governance/DOCUMENTATION_ORGANIZATION_STANDARDS.md) | How to organize documentation |
| [Project Checklist](governance/CHECKLIST.md) | Development and deployment checklist |

## 📦 Archive

Historical documentation preserved for reference:

| Document | Description | Date |
|----------|-------------|------|
| [Docker Implementation](Archive/Reports-Historical/DOCKER_IMPLEMENTATION.md) | Docker support implementation summary | 2026-01 |
| [PR Summary](Archive/Reports-Historical/PR_SUMMARY.md) | Docker PR summary | 2026-01 |
| [Visibility Assessment](Archive/Reports-Historical/REPOSITORY_VISIBILITY_ASSESSMENT.md) | Public vs private assessment | 2026-01 |

See [Archive README](Archive/README.md) for archival policy.

> **Note**: Governance documentation consolidated on 2026-01-13. See [Archive/governance-superseded](Archive/governance-superseded/README.md) for details.

## 🔍 By Topic

### Setup & Deployment

- [Setup Guide](Guides/SETUP.md)
- [Deployment Guide](Guides/DEPLOYMENT.md)
- [Docker Guide](Guides/DOCKER.md)

### API & Development

- [API Reference](Reference/API.md)
- [Development Guide](Guides/DEVELOPMENT.md)
- [Data Format](Reference/DATA_FORMAT.md)

### Security & Standards

- [Security](Reference/SECURITY.md)
- [Documentation Standards](Governance/DOCUMENTATION_ORGANIZATION_STANDARDS.md)

### AI & Automation

- [Agent Guidelines](governance/AGENTS.md) - Comprehensive guide for AI agents
- [Project Checklist](governance/CHECKLIST.md)

## Navigation

- [Documentation Hub](README.md) - Role-based navigation
- [Root README](../README.md) - Project overview
- [Contributing](../CONTRIBUTING.md) - How to contribute

---

**Last Updated**: 2026-01-13  
**Total Active Documents**: 14 (excluding archive)
