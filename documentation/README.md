# SpatialSelects.com Documentation

Welcome to the comprehensive documentation for SpatialSelects.com - your guide to discovering spatial audio music releases.

## Quick Navigation

### 👤 For New Users

- [Setup Guide](Guides/SETUP.md) - Get started with development
- [Deployment Guide](Guides/DEPLOYMENT.md) - Deploy to production
- [Docker Guide](Guides/DOCKER.md) - Docker deployment

### 🛠 For Developers

- [Development Guide](Guides/DEVELOPMENT.md) - Development workflow
- [API Reference](Reference/API.md) - REST API documentation
- [Data Format](Reference/DATA_FORMAT.md) - Track data standards
- [Security](Reference/SECURITY.md) - Security best practices

### 🤖 For AI Agents

- [Agent Guidelines](Governance/AGENTS.md) - AI agent instructions
- [Documentation Standards](Governance/DOCUMENTATION_ORGANIZATION_STANDARDS.md) - How to maintain docs
- [Project Checklist](Governance/CHECKLIST.md) - Development checklist

### 📋 Complete Index

- [Documentation Index](DOCUMENTATION_INDEX.md) - Full catalog of all documentation

## Documentation Structure

```
Documentation/
├── Guides/          # How-to guides for setup, deployment, development
├── Reference/       # API documentation and technical references
├── Governance/      # Standards, guidelines, and AI agent instructions
└── Archive/         # Historical documentation
```

## Recent Updates

- **2026-01-10**: Reorganized documentation structure - consolidated `docs/` into `Documentation/`
- **2026-01-10**: Established documentation organization standards
- **2026-01-05**: Added repository visibility assessment

## Contributing to Documentation

When updating documentation:

1. Follow [Documentation Organization Standards](Governance/DOCUMENTATION_ORGANIZATION_STANDARDS.md)
2. Keep root directory to ≤10 markdown files
3. Update this README and the index when adding new docs
4. Archive (don't delete) superseded content

## Need Help?

- Check the [complete index](DOCUMENTATION_INDEX.md)
- Review [contributing guidelines](../CONTRIBUTING.md)
- Open an issue on GitHub
